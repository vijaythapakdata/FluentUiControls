...
export default class SpfxhttpclientWebPart extends BaseClientSideWebPart<ISpfxhttpclientWebPartProps> {
  ...
  public async  render(): Promise<void> {
    if(!this.renderedOnce){
      const response:INasaImageSearchResponse=await this._getImages();
    const element: React.ReactElement<ISpfxhttpclientProps> = React.createElement(
      Spfxhttpclient,
      {
        ...
        appolloMissionImage:response.collection.items[0]
      }
    );
...
protected renderCompleted() {
  ...
  //Get Images
  public async _getImages():Promise<INasaImageSearchResponse>{
    const response:HttpClientResponse=await this.context.httpClient.get(`https://images-api.nasa.gov/search?q=Apollo%204&media_type=image`,
